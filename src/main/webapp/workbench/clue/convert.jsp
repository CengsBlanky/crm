...
<script type="text/javascript">
	$(function(){
		...
		$(window).keydown(function (event) {
			if(event.keyCode==13){
				//根据市场活动名称（aname）查找对应的活动
				var aname = $("#aname").val();
				$.ajax({
					url:"workbench/clue/searchActivityOnlyByName.do",
					data:{"aname":aname},
					type:"get",
					dataType:"json",
					success:function (aList) {
						...
						$.each(aList,function (i, activity) {
							...
							html += '<td id="n'+activity.id+'">'+activity.name+'</td>';
		...
		//为提交按钮绑定事件，将事件的名称显示在文本框中，同时在文本框下方放置一个隐藏域，存储活动的id
		$("#submitActivityBtn").click(function () {
			var $box = $("input[name='activity']:checked");
			if($box.length==0){
				alert("请选择一个活动");
				return false;
			}
...
<body>
	...
	<div id="create-transaction1" style="position: relative; left: 40px; height: 35px; top: 25px;">
		<input type="checkbox" name="isCreateTran" id="isCreateTransaction"/>
		为客户创建交易
	</div>
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
	<%---------------------------------转换表单------------------------------------------------------%>
		<form id="transForm" action="workbench/clue/convert.do">
			<input type="hidden" value="${param.id}" id="clueId" name="clueId">
			<input type="hidden" value="0" name="a" id="index">
		  ...
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		    ...
		    <input type="text" class="form-control time" id="expectedClosingDate" name="expectedClosingDate" readonly />
		  </div>
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		  ...
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		    ...
		    <input type="text" class="form-control" id="activity" name="activity" placeholder="点击上面搜索" readonly>
