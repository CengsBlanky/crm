...
<script type="text/javascript">
	...
	$(function(){
		...
		//绑定搜索按钮
		$("#findClueBtn").click(function () {
			//将数值放入隐藏域
			$("#hidden-clueName").val($("#find-name").val());
			$("#hidden-company").val($("#find-company").val());
			$("#hidden-phone").val($("#find-phone").val());
			$("#hidden-source").val($("#find-source").val());
			$("#hidden-owner").val($("#find-owner").val());
			$("#hidden-mphone").val($("#find-mphone").val());
			$("#hidden-clueState").val($("#find-clueState").val());
			disClueList(1,2);
		});
		...
		$("#createClueModalBtn").click(function () {
			$.ajax({
				url:"workbench/clue/findUsers.do",
				type:"get",
				dataType:"json",
		...
		$("#saveClueBtn").click(function () {
			$.ajax({
				...
				success:function (data) {
					if(data.success){
						//回到首页保持当前页容量，关闭模态窗口,清空模态窗口中的内容

						$("#createClueModal").modal("hide");
						$("#clueSavaModal")[0].reset();
						disClueList(1,$("#activityPage").bs_pagination('getOption', 'rowsPerPage'));
					}else{
						alert("线索添加失败")
					}
				}
	...
	//展现线索列表
	function disClueList(pageNo, pageSize) {
		var name= $("#hidden-clueName").val();
		...
		var state = $("#hidden-clueState").val();
		$.ajax({
			url:"workbench/clue/disClueList.do",
			data:{
				"pageNo":pageNo,
				"pageSize":pageSize,
				"name":name,
			...
			type:"get",
			dataType:"json",
			success:function (data) {//返回值：totalRecord,dataList(clue)
				var html="";
				$.each(data.dataList,function (i, clue) {
					html += '<tr>';
				...
				$("#activityPage").bs_pagination({
					...
					onChangePage : function(event, data){
						disClueList(data.currentPage , data.rowsPerPage);
					}
				});
			}
		});
...
<body>
	...
	<%--查询使用的数据隐藏域--%>
	<input type="hidden" id="hidden-clueName" />
		...
		<div class="modal-dialog" role="document" style="width: 90%;">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" role="form" id="clueSavaModal">
				...
				<form class="form-inline" role="form" style="position: relative;top: 8%; left: 5px;">
				  ...
				  <div class="form-group">
				    <div class="input-group">
					  ...
					  <select class="form-control" id="find-clueState">
					  	<option value="">-请选择-</option>
					  	<c:forEach items="${clueStateList}" var="clueState">
							<option value="${clueState.value}">${clueState.text}</option>
						</c:forEach>
				  ...
				  <button type="button" class="btn btn-default" id="findClueBtn">查询</button>
