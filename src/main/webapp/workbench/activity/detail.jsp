    ...
    <script type="text/javascript">
        ...
        $(function(){
            ...
            $("#updateRemarkBtn").click(function () {
                //需要上传：noteContent,id；修改时间为当前时间，修改人为当前用户，编辑状态变为1;返回值为remark和boolean
                $.ajax({
                    ...
                    success:function (data) {
                        if(data.success){
                            //将前端对应的备注内容进行修改
                            $("#h5"+data.remark.id).html(data.remark.noteContent);
                            $("#d"+data.remark.id).html(data.remark.editTime+"由"+data.remark.editBy);
                            //关闭修改备注的模态窗口
            ...
            $("#saveRemarkBtn").click(function () {
                //发送ajax请求，发送的参数为noteContent,createBy,activityId,返回的参数为success:true/false,ActivityRemark JSON对象
                ...
                $.ajax({
                    ...
                    success:function(data){
                        if(data.success){
                            //清空文本域
                            $("#remark").val("");
                            //成功则添加到备注列表中data.activityRemark
                            var html = "";
                            html += '<div id="'+data.activityRemark.id+'" class="remarkDiv" style="height: 60px;">';
                            html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
                            html += '<div style="position: relative; top: -40px; left: 40px;" >';
                            html += '<h5 id="h5'+data.activityRemark.id+'">'+data.activityRemark.noteContent+'</h5>';
                            html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activityDetail.name}</b> <small style="color: gray;" id="d'+data.activityRemark.id+'">'+data.activityRemark.createTime+' 由'+data.activityRemark.createBy+'</small>';
            ...
            function activityRemark() {
                ...
                $.ajax({
                    ...
                    type: "get",
                    dataType: "json",
                    success: function (arList) {
                        //备注信息
                        var html = "";
                        $.each(arList, function (i,remark) {
                            html += '<div id="'+remark.id+'" class="remarkDiv" style="height: 60px;">';
                            html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
                            html += '<div style="position: relative; top: -40px; left: 40px;" >';
                            html += '<h5 id="h5'+remark.id+'">'+remark.noteContent+'</h5>';
                            html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activityDetail.name}</b> <small style="color: gray;" id="d'+remark.id+'">'+(remark.editFlag==0?remark.createTime:remark.editTime)+' 由'+(remark.editFlag==0?remark.createBy:remark.editBy)+'</small>';
                            html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
        ...
        //点击备注编辑图标,将原备注内容显示出来，同时在隐藏域保存备注ID
